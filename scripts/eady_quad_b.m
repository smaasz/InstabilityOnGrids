...
% instability
  ...
  %U=-(M2*dz/f0)*cos(thetaw)*[-(1:Nz)+1/2];
  %V=-(M2*dz/f0)*sin(thetaw)*[-(1:Nz)+1/2];
 ...
 for K=Kvec,   % for K=0 sometimes there are problems with
     ...
     fx=1+(1/3)*(1-cos(k)); fy=1+(1/3)*(1-cos(l));
     kappa=0.002*a^3;
     Lapl=gx*conj(gx)+gy*conj(gy);
     nu=kappa*Lapl^2;
     %ax=1; ay=1;
     %%%% Coefficients in the derivatives of b in b equation
     
     %%%% Matrices acting on u and v in the u and v equations
     Au=diag(ax*gx*U*fx+ay*gy*V*fy+nu)-(M2/f0)*cos(thetaw)*ax*ay*(-gx)*W;
     %Au=diag(i*k*U/a+i*l*V/a)-(M2/f0)*cos(thetaw)*ax*(-gx)*W;
     
     Av=-f0*diag(ones([1,Nz]))-(M2/f0)*cos(thetaw)*ax*ay*(-gy)*W;
     % +gx*P b +g*gx eta
     Bv=diag(ax*gx*U*fx+ay*gy*V*fy+nu)-(M2/f0)*sin(thetaw)*ay*ax*(-gy)*W;
     %Bv=diag(i*k*U/a+i*l*V/a)-(M2/f0)*sin(thetaw)*ay*(-gy)*W;
     
     Bu=f0*diag(ones([1,Nz]))-(M2/f0)*sin(thetaw)*ay*ax*(-gx)*W;
     % +gy*P b +g*gy eta
     Cb=diag(ax*gx*U*fx+ay*gy*V*fy+nu);
     %Cb=diag(i*k*U/a+i*l*V/a);
     Cu=N2*W*(-gx*ay)+(bx*ax*ay)*diag(ones([1,Nz]));
     Cv=N2*W*(-gy*ax)+(by*ay*ax)*diag(ones([1,Nz]));
     
     %%% sea surface height matrix
     Eu=g*gx*ay*ones([Nz,1]);
     Ev=g*gy*ax*ones([Nz,1]);
     Du=dz*gx*ay*ones([1,Nz]);
     Dv=dz*gy*ax*ones([1,Nz]);
     %%% Construct system matrix:
     S=-[Au,Av,gx*ay*P,Eu; Bu,Bv,gy*ax*P,Ev;Cu, Cv,Cb,zeros([Nz,1]);Du,Dv,zeros([1,Nz+1])];
...
% Summary: Exact advection and exact Coriolis eliminate the mode. Averaging
% in the M2 terms does not matter. However, if the advection is of
% the second order, exact Coriolis does not help. If only the
% advection of buoyancy is exact, the exactness of Coriolis does not help a
% lot, but the instability is suppressed except for the vicinity of cutoff.
