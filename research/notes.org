...
The Fourier symbol of the linearized advection operator may be
expressed in terms of three contributions of different origin.
**** The vertical advection of mean buoyancy:
$\mathrm{N}^2 \mathrm{Av}_z\Hat{w}$,
...
the Fourier symbol has the following form.
\begin{equation}
\begin{bmatrix}
...
$G_a^{(xy)} = G_a^{(yx)} = \boldsymbol{0}$ for both $a=x,y$ but the 
