...
#+LATEX_HEADER: \usepackage{mathtools} \usepackage{bm} \usepackage{tikz} \usetikzlibrary {shapes.geometric} \DeclareMathOperator{\uhma}{Uhma_c^c}  \DeclareMathOperator{\vhma}{Vhma_c^c} \newcommand{\gradcc}{\Vec{\mathrm{Grad}}_\mathrm{c}^\mathrm{c}} \DeclareMathOperator{\gradcv}{Grad_v^c}\DeclareMathOperator{\avcv}{Av_v^c} \newcommand{\bsu}{\boldsymbol{u'}} \newcommand{\bsv}{\boldsymbol{v'}} \newcommand{\bsb}{\boldsymbol{b'}} \newcommand{\bsp}{\boldsymbol{p'}} \newcommand{\bseta}{\boldsymbol{\eta'}} \newcommand{\bsw}{\boldsymbol{w'}} \newcommand{\abs}[1]{\lvert #1\rvert} \DeclareMathOperator{\Gop}{\underline{G}} \DeclareMathOperator{\Wop}{\underline{W}} \DeclareMathOperator{\Pop}{\underline{P}} \DeclareMathOperator{\Aop}{\underline{A}} \DeclareMathOperator{\Gvop}{\underline{\cal G}} \DeclareMathOperator{\Dop}{\underline{D}}

* Title
Effects of horizontal discretization on triangular and hexagonal grids on linear baroclinic and symmetric instabilities
* Abstract
As global ocean general circulation models are run at eddy-permitting
resolutions, reproducing accurate growth rates of baroclinic
instabilities is a major concern when choosing a discretization of the
equations of motion. From this viewpoint, discretizations on
triangular and hexagonal grids with different types of variable
staggering are analyzed, as several well-established models use
formulations on (possibly) unstructured meshes. By extending the
linear baroclinic instability analysis in the Eady configuration by
Barham et al. to discretizations on more complex grids, several
numerical subtleties are revealed. However, it is shown that in each
case, moderate biharmonic dissipation suppresses spurious branches on
the baroclinic axis.  In comparison to discretizations on
quadrilateral grids, the analyzed discretizations are less robust
against spurious modes, as expected due to additional geometric
modes. Some of the subtleties arise because discretizations on
triangular and hexagonal grids do not adhere to Galilean
invariance. As a consequence, the spurious branches of instability
demonstrate a dependence on the alignment between the background flow
and the grid lattices, as well as the strength of a uniform background
flow. The interactions with spurious modes become more significant on
the axis of symmetric instabilities, where the physical and spurious
branches of instability are even more difficult to separate in
wavenumber space. Our analysis illustrates the need to carefully
calibrate the viscosity and diffusivity parameters for each of the
considered discretizations in order to suppress spurious branches of
instabilities.
...
\cite{Barham2018}, we use the hydrostatic and the Boussinesq
approximations, and consider the buoyancy as a single scalar field. We
also analyze the effect of several momentum advection schemes,
...
located at the centroids of the triangles. In a finite volume
formulation their values may be interpreted as the mean on the
resprective triangles which are called \textit{cells} in this
context. The scalar quantities such as surface height, pressure,
buoyancy and vertical velocity are staggered with respect to the
...
contains two grid points of horizontal velocity. In particular, these
grid points form two hexagonal lattices. The mismatch in degrees of
freedom allows for spurious/geometric modes that need to be dissipated
by an appropriate viscosity operator \cite{???}.
...
formulated in terms of discrete analogs of the differential operators.
The discrete operators may be interpreted as (the sum of pointwise
products of) discrete convolutions on these lattices. The kernels may
be defined on a lattice ismorphic to the lattice of the respective
input signal but shifted to a collocation point of the respective
output signal. The Fourier symbol of a discrete operator corresponding
to a constant coefficient convolution kernel is then given by the discrete
Fourier transform of the respective convolution kernels.

Using the notation of \cite{Danilov2017} (see table ...)  discrete
analogs of the gradient of a scalar quantity, the divergence of a
flux, and the curl of the horizontal velocity are given by the
...
= \frac{1}{A_v} \sum_{e\in E(v)}d_e\frac{1}{2} \sum_{c\in C(e)} \Vec{n}_{v,e} \cdot \Vec{F}_c
\end{equation}
\begin{equation}
\big( \nabla \boldsymbol{\Vec{F}}\big)_c
=\sum_{e\in E(c)} \frac{2}{3} \Vec{n}_{c,e} \frac{1}{d_e}\sum_{\Tilde{c}\in C(e)} \Vec{n}_{c,e}^T\Vec{n}_{\Tilde{c},e}  \Vec{F}_v^T
\end{equation}
\begin{equation}
...
The discrete gradient operator acting on the pressure field averages
the pressure values along edges and identifies the gradient with the
normal to the unique plane defined by the pressure averages on the
three boundary edges of the respective cell. On a structured grid the
Fourier symbol of the operator has the form $$\begin{bmatrix} g_x^u &
g_x^d & g_y^u & g_y^d\end{bmatrix}^T$$ where $g_x$ and $g_y$ are the
components of the gradient and the superscript indicates the lattice
of upward-pointing and downward-pointing cells respectively. Due to
symmetry the respective components of the gradient on the two lattices
are related: $g^d = -\left(g^u\right)^*$. The Fourier symbol of the
gradient on the lattice of upward-pointing cells satifies the
following relation
\begin{equation}
\begin{bmatrix}
g_x^u \\ g_y^u
\end{bmatrix}=
\begin{bmatrix}
\frac{ - \cos\left(  - \frac{1}{2} a k - \frac{1}{3} h l \right) + \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{a} + \frac{ - \sin\left(  - \frac{1}{2} a k - \frac{1}{3} h l \right) + \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{a} \mathit{i}\\
\frac{ - \cos\left(  - \frac{1}{2} a k - \frac{1}{3} h l \right) + 2 \cos\left( \frac{2}{3} h l \right) - \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{2 h} + \frac{ - \sin\left(  - \frac{1}{2} a k - \frac{1}{3} h l \right) + 2 \sin\left( \frac{2}{3} h l \right) - \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{2 h} \mathit{i}
\end{bmatrix}
\end{equation}


On a structured grid the discrete gradient operator acting on a vector
field can be easily expressed by \eqref{...} and corresponds to the
more general least square formulation on unstructured grids
(cf. \cite{Danilov2017}). Its Fourier symbol in the x-direction can be
expressed in terms of the previously discussed gradient's Fourier
symbols and its skew-hermitian property should be noted
...
The discrete divergence operator acting on a flux field
averages the normal fluxes along edges to compute the total flux
through the six boundary edges of the scalar control volume, $v$. The
Fourier symbol of the discrete divergence operator is related to the
negative complex conjugate of the gradient's Fourier symbol

\begin{equation}
\begin{bmatrix}-\left(g_x^u\right)^*/2 & -\left(g_x^d\right)^*/2  & -\left(g_y^u\right)^*/2 & -\left(g_y^d\right)^*/2\end{bmatrix}.
\end{equation}
...
been proposed \cite{???}. Here the analysis is restricted to four
different schemes. Figure \ref{...} shows the eigenvalues of the
corresponding Fourier symbols evaluated at wavevectors in the
along-front direction.

[[../plots/hmt-FS.png]]

Similarly, the transport of buoyancy may be either discretized in the
flux form or the advective form. In this study the operatoration is
formulated in flux-form but in analogy to the transport of momentum
the flux-form is reduced to the advective-form by disregarding the
transport due to the mismatch in the divergence discretization.

The vertical advection of buoyancy only requires the averaging from
mid-level faces to full levels but no further averaging as the vertical
velocity and buoyancy are horizontally collocated. 
\begin{equation}
\Big( w\partial_z \boldsymbol{b}\Big)_{c,k} = \frac{1}{2} \left(w_{c,k+1/2} \frac{b_{c,k+1} - b_{c,k}}{\Delta_z} + w_{c,k-1/2} \frac{ b_{c,k} - b_{c,k-1} }{\Delta_z}\right)
\end{equation}

A general approach to express the horizontal buoyancy transport is to
reconstruct the velocity and buoyancy field to edges so that the
operator may simply expressed in terms of the buoyancy flux across the
bounding edges of the scalar control volume, $v$, as follows

\begin{equation}
A_v\Big(\nabla \cdot \big(\boldsymbol{\Vec{u}}\boldsymbol{b}\big) \Big)_{v}
&=\sum_{e\in E(v)} d_e \Vec{n}_{v,e} \Vec{u}_e b_e.
\end{equation}
Danilov et al. chose the low-order accurate reconstruction of the
advecting horizontal velocity field given by averaging along the
respective edge but used a higher-order accurate reconstruction for
the advected buoyancy. There are two extreme choices corresponding to
a centered and upwind reconstruction respectively but in practice a convex
combination of both is chosen. \cite{Danilov2017}

Here, the higher-order accurate scheme is contrasted with a
lower-order accurate scheme where the buoyancy field is simply
averaged across the respective edge $b_e = \sum_{v\in V(e)} b_v/2$.

The Fourier symbol of the linearized advection operator acting on the
buoyancy may be expressed in terms of three contributions of different
origin.
**** The vertical advection of mean buoyancy:
Since the vertical velocity and buoyancy are collocated at the
vertices of the triangular grid, no additional averaging other than in
vertical direction due to the Lorenz-type staggering is needed. The
contribution to the Fourier symbol involves the Brunt-Väisälä
frequency of background flow: $\mathrm{N}^2 \mathrm{Av}_z\Hat{w}$.
**** The horizontal advection of mean buoyancy:
Due to the staggering of the horizontal velocity with respect to the
buoyancy the horizontal velocity requires averaging in order to
comptue the directional derivative of mean buoyancy along the velocity
field. In fact, on a triangular grid it turns out that there are
cross-interactions; i.e. the x-component of the horizontal velocity
also advects the change of mean buoyancy in the y-direction and vice
versa. In particular, in the Eady configuration also the along-front
velocity contributes to the advection of mean buoyancy. As a result
the contribution to the Fourier symbol has the following form.
\begin{equation}
\begin{bmatrix}
 - \mathtt{Axx} \mathtt{M{^2}} \sin\left( \mathtt{{\theta}U} \right) + \mathtt{Axy} \mathtt{M{^2}} \cos\left( \mathtt{{\theta}U} \right) &  - \mathtt{Ayx} \mathtt{M{^2}} \sin\left( \mathtt{{\theta}U} \right) + \mathtt{Ayy} \mathtt{M{^2}} \cos\left( \mathtt{{\theta}U} \right)
\end{bmatrix}
\end{equation}
**** The horizontal advection of buoyancy by the background flow:
The use of an higher-order accurate reconstruction scheme for the
buoyancy yields an improved accuracy for the buoyancy's rate of change
along the flow direction. Figure \ref{...} illustrates the difference
in the Fourier symbols evaluated at wavevectors in the flow direction
of the respective schemes. However, due to the staggering of
horizontal velocity and buoyancy the Fourier symbol differs from the
Fourier symbol in the advection of horizontal momentum ($Ug_x + Vg_y$).

[[../plots/hstb-FS.png]]

**** advective form
On a Lorenz grid the advection of the horizontal velocity by the
vertical velocity requires the averaging from mid-level faces to the
full levels. On a B-type staggering additional averaging of the
vertical velocity to cells is needed.
\begin{equation}
\Big( w\partial_z \boldsymbol{\Vec{u}}\Big)_{c,k} = \frac{1}{2} \left((\mathrm{Av} w)_{c,k+1/2} \frac{\Vec{u}_{c,k+1} - \Vec{u}_{c,k}}{\Delta_z} + (\mathrm{Av} w)_{c,k-1/2} \frac{ \Vec{u}_{c,k} - \Vec{u}_{c,k-1} }{\Delta_z}\right).
\end{equation}
The following simple scheme might be used for the advection by the
horizontal velocity field.
***** streamline directional derivative using the vector gradient (ASC)
...
\end{equation*}
The Fourier symbols of its linearization around a flow with constant
velocities on horizontal planes is skew-hermitian but not diagonal. In
particular, the addition of constant flow will yield a non-diagonal
...
\end{bmatrix}
\end{equation}
More generally, write the Fourier symbol of the linearized advection
operator acting on horizontal velocity may be written in the form
$UG_x + VG_y$ where in this case the following relations hold.
\begin{align*}
G_a^{(xx)} = G_a^{(yy)} = \begin{bmatrix} 0 & g_a\\ g_a & 0 \end{bmatrix} & G_a^{(xy)} = G_a^{(yx)} = \boldsymbol{0} & a=x,y
\end{align*}
**** flux form
...
vertical velocities to cell centers. In this study the average of the
horizontal velocities of the cells adjacent to the face is used as
reconstruction.
\begin{align*}
A_c\Delta_z\Big( \partial_z \big(\boldsymbol{w} \boldsymbol{\Vec{u}}\big)\Big)_{c,k}
...
&= A_c\Delta_z \left[\Vec{u}_{c,k} \left(\mathrm{Av}\left( -\nabla\cdot \Vec{u}\right)_{v,k}\right)_{c,k} + \frac{1}{2} \left((\mathrm{Av} w)_{c,k+1/2} \frac{\Vec{u}_{c,k+1} - \Vec{u}_{c,k}}{\Delta_z} + (\mathrm{Av} w)_{c,k-1/2} \frac{ \Vec{u}_{c,k} - \Vec{u}_{c,k-1} }{\Delta_z}\right)\right]
\end{align*}

The discretization of the horizontal momentum flux through the
vertical faces of the cell can be (1) either computed on scalar
control volumes and then averaged to cells, or (2) directly computed
on cells in which case two reconstructions for the \textit{advecting}
and \textit{transported} horizontal velocities onto the faces of the
cell must be specified.

***** divergence on scalar control volumes (FDV)
...
must be averaged from the scalar control volumes back to the
cells.
#+NAME: hma-flux-form
\begin{equation*}
...
Analyzing the Fourier symbol of the linearized operator as above it
should be noted that the off-diagonal submatrices vanish $G_a^{(xy)} =
G_a^{(yx)} = \boldsymbol{0}$ for both $a=x,y$ but the diagonal
submatrices are full.

***** divergence on cells with vector reconstructions onto edges (FDCRE)
...
the linearized operator vanish but the diagonal submatrices are
full. The physical branch of the eigenvalues is more accurate than for
the other branches. There is, however, a spurious/geometric mode in
the opposite direction of the advecting background flow (see Figure \ref{...}).

***** Reduction of flux to advective form
Assume the following discretization of the flux form
\begin{equation*}
\Delta_z\sum_{e\in E(c)} \ell_e \Vec{n}_{c,e} \cdot \Vec{u}_{\mathrm{ad},e}\Vec{u}_{\mathrm{tr},e} + A_c\Delta_z\Big( \partial_z \big(\boldsymbol{w} \boldsymbol{\Vec{u}}\big)\Big)_{c,k}
...
\end{equation*}
Using the continuity equation \eqref{...} the flux form may be transformed into an advective form
\begin{equation*}
\Delta_z\sum_{e\in E(c)} \ell_e \Vec{n}_{c,e} \cdot \Vec{u}_{\mathrm{ad},e}\left(\Vec{u}_{\mathrm{tr},e} - \Vec{u}_{c,k}\right) + A_c\Delta_z\Vec{u}_{c,k}\Bigg(\frac{1}{A_c}\sum_{e\in E(c)} \ell_e \Vec{n}_{c,e} \cdot \Vec{u}_{\mathrm{ad},e} - \left(\mathrm{Av}\left( \nabla\cdot \Vec{u}\right)_{v,k}\right)_{c,k}\Bigg) + A_c\Delta_z \Big( w\partial_z \boldsymbol{\Vec{u}}\Big)_{c,k}.
\end{equation*}

The second term may be interpreted as the momentum transport due to a
non-vanishing divergence which originates from the mismatch of
horizontal divergence discretization quantified in the last factor of
the term. In this study, the advective form is used by neglecting the
discussed second term.

Note that an analogous discussion is needed for the flux-form formulation of the
buoyancy equation.
...
\end{equation*}
In the Fourier symbol of the linearized operator the submatrices
$G_a^{(xx)}$ and $G_a^{(yy)}$ are full for both $a=x,y$. In addition
the cross-interaction submatrices $G_x^{(yx)}$ and $G_y^{(xy)}$ are
non-zero and real.  Interestingly, the following relations between the Fourier
symbols in the different schemes hold.

\begin{align*}
G_x^{(xx)}(\mathrm{AVI}) = G_x^{(xx)}(\mathrm{FDV}) + G_y^{(xy)}(\mathrm{AVI}) && G_x^{(yy)}(\mathrm{AVI}) = G_x^{(yy)}(\mathrm{FDV})\\
G_y^{(yy)}(\mathrm{AVI}) = G_y^{(yy)}(\mathrm{FDV}) + G_x^{(yx)}(\mathrm{AVI}) && G_y^{(xx)}(\mathrm{AVI}) = G_y^{(xx)}(\mathrm{FDV})
\end{align*}

\begin{equation}
G_x^{(xy)}(\mathrm{AVI}) = 
\begin{bmatrix}
\frac{\cos\left( \frac{1}{2} a k - h l \right) - \cos\left(\frac{1}{2} a k + h l \right)}{3 a} & 0\\
0 & -\frac{\cos\left( \frac{1}{2} a k - h l \right) - \cos\left(\frac{1}{2} a k + h l \right)}{3 a}
\end{bmatrix} =
\begin{bmatrix}
\frac{1}{3} k l h & 0\\
0 & -\frac{1}{3} k l h
\end{bmatrix} + \mathcal{O}\left(h^3\right)
\end{equation}

\begin{equation}
G_y^{(yx)}(\mathrm{AVI}) = 
\begin{bmatrix}
\frac{\cos\left( \frac{1}{2} a k - h l \right) - 2 \cos\left(  - a k \right) + U \cos\left( \frac{1}{2} a k + h l \right) }{6 h} & 0\\
0 & \frac{\cos\left( \frac{1}{2} a k - h l \right) - 2 \cos\left(  - a k \right) + U \cos\left( \frac{1}{2} a k + h l \right) }{6 h}
\end{bmatrix} =
\begin{bmatrix}
\frac{1}{6} (k^2 -  l^2) h & 0\\
0 & \frac{1}{6} (k^2 -  l^2) h
\end{bmatrix} + \mathcal{O}\left(h^3\right)
\end{equation}
...
\left[
\begin{array}{cc}
\frac{ - \cos\left( \frac{1}{2} a k - h l \right) + \cos\left( \frac{1}{2} a k + h l \right)}{6 a} + \frac{\sin\left( \frac{1}{2} a k + h l \right) + 2 \sin\left( a k \right) + \sin\left( \frac{1}{2} a k - h l \right)}{6 a} \mathit{i} & \frac{\cos\left( \frac{1}{2} a k + \frac{1}{3} h l \right) + \cos\left( a k - \frac{2}{3} h l \right) - \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right) - \cos\left( a k + \frac{2}{3} h l \right)}{6 a} + \frac{\sin\left( a k - \frac{2}{3} h l \right) + \sin\left( a k + \frac{2}{3} h l \right) + \sin\left( \frac{1}{2} a k + \frac{1}{3} h l \right) + \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{6 a} \mathit{i} \\
\frac{ - \cos\left( \frac{1}{2} a k + \frac{1}{3} h l \right) - \cos\left( a k - \frac{2}{3} h l \right) + \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right) + \cos\left( a k + \frac{2}{3} h l \right)}{6 a} + \frac{\sin\left( a k - \frac{2}{3} h l \right) + \sin\left( a k + \frac{2}{3} h l \right) + \sin\left( \frac{1}{2} a k + \frac{1}{3} h l \right) + \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{6 a} \mathit{i} & \frac{\cos\left( \frac{1}{2} a k - h l \right) - \cos\left( \frac{1}{2} a k + h l \right)}{6 a} + \frac{\sin\left( \frac{1}{2} a k + h l \right) + 2 \sin\left( a k \right) + \sin\left( \frac{1}{2} a k - h l \right)}{6 a} \mathit{i}
\end{array}
\right]\\
&=
\left[
\begin{array}{cc}
- \frac{1}{6} h k l + k \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}  &  \frac{1}{6} h k l + k \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}\\
- \frac{1}{6} h k l + k \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i} &   \frac{1}{6} h k l + k \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}
\end{array}
...
\left[
\begin{array}{cc}
\frac{ - \cos\left( \frac{1}{2} a k - h l \right) - \cos\left( \frac{1}{2} a k + h l \right) + 2 \cos\left( a k \right)}{12 h} + \frac{\sin\left( \frac{1}{2} a k + h l \right) - \sin\left( \frac{1}{2} a k - h l \right)}{4 h} \mathit{i} & \frac{ - 2 \cos\left( \frac{2}{3} h l \right) + \cos\left( \frac{1}{2} a k + \frac{1}{3} h l \right) - \cos\left( a k - \frac{2}{3} h l \right) + 2 \cos\left( \frac{4}{3} h l \right) + \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right) - \cos\left( a k + \frac{2}{3} h l \right)}{12 h} + \frac{ - \sin\left( a k - \frac{2}{3} h l \right) + 2 \sin\left( \frac{4}{3} h l \right) + \sin\left( a k + \frac{2}{3} h l \right) + 2 \sin\left( \frac{2}{3} h l \right) + \sin\left( \frac{1}{2} a k + \frac{1}{3} h l \right) - \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{12 h} \mathit{i} \\
\frac{2 \cos\left( \frac{2}{3} h l \right) - \cos\left( \frac{1}{2} a k + \frac{1}{3} h l \right) + \cos\left( a k - \frac{2}{3} h l \right) - 2 \cos\left( \frac{4}{3} h l \right) - \cos\left( \frac{1}{2} a k - \frac{1}{3} h l \right) + \cos\left( a k + \frac{2}{3} h l \right)}{12 h} + \frac{ - \sin\left( a k - \frac{2}{3} h l \right) + 2 \sin\left( \frac{4}{3} h l \right) + \sin\left( a k + \frac{2}{3} h l \right) + 2 \sin\left( \frac{2}{3} h l \right) + \sin\left( \frac{1}{2} a k + \frac{1}{3} h l \right) - \sin\left( \frac{1}{2} a k - \frac{1}{3} h l \right)}{12 h} \mathit{i} & \frac{\cos\left( \frac{1}{2} a k - h l \right) + \cos\left( \frac{1}{2} a k + h l \right) - 2 \cos\left( a k \right)}{12 h} + \frac{\sin\left( \frac{1}{2} a k + h l \right) - \sin\left( \frac{1}{2} a k - h l \right)}{4 h} \mathit{i}
\end{array}
\right]\\
&=
\left[
\begin{array}{cc}
- \frac{1}{12} \left(k^2 - l^2) h + l \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}  & \frac{1}{12} \left(k^2 - l^2) h + l \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i} \\
- \frac{1}{12} \left(k^2 - l^2) h + l \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}  & \frac{1}{12} \left(k^2 - l^2) h + l \left( \frac{1}{2} - \frac{1}{12} \left( k^{2} + l^2 \right) h^{2} \right) \mathit{i}
\end{array}
\right] + \mathcal{O}\left( h^3 \right)
\end{align*}
...
\begin{equation}
\begin{bmatrix}
U\Gop_x^{(xx)} + V\Gop_y^{(xx)} + U_z\Aop_v^c \Wop^{(x)} & U\Gop_x^{(xy)} + V\Gop_y^{(xy)} + U_z\Aop_v^c \Wop^{(y)} - f_0 \mathrm{\underline{I}} & \underline{g}_x\Pop & g \mathrm{\underline{1}} g_x\\
U\Gop_x^{(yx)} + V\Gop_y^{(yx)} + U_z\Aop_v^c \Wop^{(x)} + f_0 \mathrm{\underline{I}} & U\Gop_x^{(yy)} + V\Gop_y^{(yy)} + U_z\Aop_v^c \Wop^{(y)} & \underline{g}_x\Pop & g \mathrm{\underline{1}} g_x\\
N^2 \Wop^{(x)} + B_x \Aop^{(xx)} + B_y \Aop^{(yx)} & N^2 \Wop^{(y)} + B_x \Aop^{(xy)} + B_y \Aop^{(yy)} & U \Gvop_x + V\Gvop_y & \underline{0}\\
\Delta_z\underline{1}^T \Dop^{(x)} & \Delta_z\underline{1}^T \Dop^{(y)} & \underline{1}^T\underline{0} & 0
\end{bmatrix}
\end{equation}
...
the horizontal faces on a Lorenz grid reconstructs the (transported)
horizontal velocities onto the mid-level faces and averages the
vertical velocities to cell centers. Here the average of the
horizontal velocities of the cells adjacent to the face is used as
reconstruction.
\begin{align*}
&A_c \left[(\mathrm{Av} w)_{c,k+1/2}(\mathrm{Av} \Vec{u})_{c,k+1/2}-(\mathrm{Av} w)_{c,k-1/2}(\mathrm{Av} \Vec{u})_{c,k-1/2}\right]\\
...
How can the mismatch of divergence discretization yield unstable
modes?
...
and downwind dual control volumes. In both cases the reconstruction of
the buoyancy onto the edge of a scalar control volume is obtained by
an interpolation scheme that is third-order accurate on a regular
mesh. On a regular mesh the interpolation scheme can be interpreted as
fourth-order accurate for $\gamma=1$.
#+NAME: hsa-flux-form
\begin{subequations}
\begin{align*}
...
\end{align*}
\end{subequations}
A computationally less expensive discretization is obtained by
replacing the reconstruction with a first-order accurate interpolation
scheme.
#+NAME: hsa-flux-form-low
\begin{subequations}
\begin{align*}
F_\mathrm{vol} &= \ell_{\Hat{e}} \sum_{c\in C(e)} \Vec{n}_{\Tilde{v},e} \cdot \Vec{u}_c / 2\\
db_{\mathrm{cen}} &= \frac{1}{\ell_e}\sum_{\hat{v}\in V(e)} \Vec{n}_{\Hat{v},e} \cdot \Vec{n}_{\Tilde{v},e} b_{\Hat{v}} \\
b_{\mathrm{tr}} &= b_{\Tilde{v}} + \frac{\ell_e}{2} db_{\text{cen}}\\
A_v\Big(\nabla \cdot \big(\boldsymbol{\Vec{u}}\boldsymbol{b}\big) \Big)_{v}
&=\sum_{e\in E(v)} \sum_{\Tilde{v}\in V(e)} \Vec{n}_{v,e}\cdot \Vec{n}_{\Tilde{v},e} \frac{F_\mathrm{vol} + \abs{F_\mathrm{vol}}}{2} b_\mathrm{tr}\\
\end{align*}
\end{subequations}
****** TODO Clarify as I calculated it to be only a 2nd-order accurate interpolation scheme
**** horizontal viscosity
A common simplification is to use the full velocity gradient instead
of only its symmetric part to estimate the strain-rate tensor (see
Griffies 2004 \cite{} for a discussion). The divergence of the
horizontal velocity gradient is then approximated by first computing
the vector gradient of the horizontal velocity field normal to the
edges of the cell and then use the divergence theorem on the cell.
\begin{equation*}
\Big(\Vec{\Delta}\boldsymbol{\Vec{u}}\Big)_c = \frac{1}{A_c} \sum_{e\in E(c)} \ell_e \sum_{\Tilde{c}\in C(e)} \frac{-\Vec{n}_{e,c}\cdot \Vec{n}_{e,\Tilde{c}}}{\ell_{\Hat{e}}} \boldsymbol{\Vec{u}}_{\Tilde{c}}
\end{equation*}
The viscosity operator based on this approximation is called \textit{harmonic}.
\begin{equation*}
\mathrm{D^u_h} = \nu_h\Big(\Vec{\Delta}\boldsymbol{\Vec{u}}\Big)_c
\end{equation*}
The harmonic viscosity coefficient is commonly scaled by the first power in the lattice parameter.
\begin{equation*}
\nu_h = V \ell_e.
\end{equation*}
Applying the calculation twice yields the \textit{biharmonic}
viscosity operator where the biharmonic viscoisty coefficient is
scaled by the third power in the lattice parameter
\begin{equation*}
\mathrm{D^u_h} = -\nu_{\text{bh}}\Big(\Vec{\Delta}^2\boldsymbol{\Vec{u}}\Big)_c
\end{equation*}
\begin{equation*}
\nu_h = V \ell_e^3.
\end{equation*}
...
Using the previous abbreviation for the linearized discretizations of
the horizontal momentum and scalar transport the system without
vertical discretization is given by the following equations.

#+NAME: eq:system-triB
\begin{subequations}
\begin{align}
\frac{\partial\bsu}{\partial t} + \Bar{U} \mathcal{L}^{(x)}_{\theta_U}\Vec{\bsu} - \cos(\theta_U) \frac{M^2}{f_0}\mathrm{Av}\bsw - f_0 \bsv &= -\Vec{e_1} \cdot \Big(\nabla(\bsp + \bseta) + \mathrm{D^u_h}\bsu\Big)\\
\frac{\partial\bsv}{\partial t} + \Bar{U} \mathcal{L}^{(y)}_{\theta_U}\Vec{\bsu} + \sin(\theta_U) \frac{M^2}{f_0}\mathrm{Av}\bsw + f_0 \bsu &= -\Vec{e_2} \cdot \Big(\nabla(\bsp + \bseta) + \mathrm{D^u_h}\bsu\Big)\\
\partial_z \boldsymbol{p}' &= \boldsymbol{b}'\\
\nabla\cdot \boldsymbol{\Vec{u}'} + \partial_z \boldsymbol{w'} &= 0\\
\frac{\partial\bsb}{\partial t} + \Bar{U} \mathcal{S}^{(b)}_{\theta_U} \boldsymbol{b'} + M^2 \mathcal{S}^{(u)}_{\theta_U}\boldsymbol{\Vec{u}'}  + N^2 \bsw & = \mathrm{D^b_h}\bsb \\
\frac{\partial\bseta}{\partial t} + \Bar{U}(z=0) \mathcal{S}^{(\text{ad})}_{\theta_U} \boldsymbol{\eta'} + \int_{-H}^0 \nabla\cdot \boldsymbol{\Vec{u}'} &= 0
\end{align}
\end{subequations}

\begin{equation*}
\boldsymbol{p}' = \int_0^z \bsb \mathrm{d}z - \frac{1}{H} \int_0^{-H} \bsb\mathrm{d}z
\end{equation*}
...
grid points of the dual mesh in the primitive model; in particular,
the vorticity is computed on the Voronoi cells. The grid points
of the horizontal velocity components are staggered with respect to
introduced grid points and with respect to each other. They are
located at the midpoints of the (primal) grid edges. The value of the
horizontal velocity component on each such point is interpreted as the
mean of the normal component of the horizontal velocity on the
respective edge (with a given orientation).
...
| $\Vec{n}_{c,e}$ | normal to cell c at face e                    |
| $\Vec{n}_{v,e}$ | normal to dual control volume v at face e     |
| $\Vec{n}_{e}$   | orientation of the face (edge) e              |
| $\Vec{t}_e$     | orientation of the dual face (edge) $\Hat{e}$ |
| $\ell_e$        | area (length) of the face (edge) e            |
| $E(c)$          | set of faces of cell c                        |
| $E(v)$          | set of faces of dual control volume v         |
| $C(e)$          | set of cells with face e                      |
...
A horizontal discretization on a triangular grid with C-type
staggering has been formulated by Korn in \cite{Korn} and is
implemented in the ocean and atomosphere components of
the\texttt{ICON} model \cite{?}. The discretization approach is based
on a weak formulation of the equations of motion and the
discretization schemes of the differential operators apply the
\textit{mimetic approach}.

Again, in the language of crystallography the collocation points of
the dual cells form a hexagonal lattice. A primitive unit cell of the
lattice contains two cell centers. In particular, the cell centers
form two hexagonal lattices. Moreover, a primitive unit cell contains
three edge centers that form three hexagonal lattices.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \draw (0,0) -- (10,0);
        \draw (0,0) -- (5,{sqrt(3)/2*10});
        \draw (0,0) -- (5,{-sqrt(3)/2*10});
        \draw (10,0) -- (5,{-sqrt(3)/2*10});
        \draw (10,0) -- (5, {sqrt(3)/2*10});
        \node[circle, color=red, fill=red, minimum size=20pt] (v1) at (0,0) {};
        %\node[draw, circle, color=red, fill=red!10, minimum size=20pt] (v1u) at (5,{sqrt(3)/2*10}) {};
        %\node[draw, circle, color=red, fill=red!10, minimum size=20pt] (v1d) at (10,0) {};
        %\node[draw, circle, color=red, fill=red!10, minimum size=20pt] (v1r) at (5,{-sqrt(3)/2*10}) {};
        \node[diamond, color=blue, fill=blue, aspect=1, minimum size=30pt] (c1) at (5,{1/(sqrt(3)*2)*10}) {};
        \node[diamond, color=purple, fill=purple, aspect=1, minimum size=30pt] (c1) at (5,{-1/(sqrt(3)*2)*10}) {};
        \node[rectangle, color=orange, fill=orange, minimum height=20pt, minimum width=20pt] (e2) at (2.5, {sqrt(3)/4*10}) {};
        %\node[draw, rectangle, color=orange, fill=orange!10, minimum height=20pt, minimum width=20pt] (e2) at (7.5, {-sqrt(3)/4*10}) {};
        \node[rectangle, color=green, fill=green, minimum height=20pt, minimum width=20pt] (e1) at (2.5, {-sqrt(3)/4*10}) {};
        %\node[draw, rectangle, color=green, fill=green!10, minimum height=20pt, minimum width=20pt] (e1) at (7.5, {sqrt(3)/4*10}) {};
        \node[rectangle, color=yellow, fill=yellow, minimum height=20pt, minimum width=20pt] (e3) at (5, 0) {};
    \end{tikzpicture}
}
    \caption{Primitive unit cell of a hexagonal lattice. The red circle is the representative of the hexagonal lattice in the unit cell. The unit cell contains two triangle centers that form two hexagonal lattices represented by the blue and purple diamond respectively. Moreover, the unit cell contains three edge centers that form three hexagonal lattices which are represented by the orange, yellow, and green square respectively.}
...
The Perot reconstructions satisfy the properties that $\mathcal{M}:=
\mathcal{P}^\top\mathcal{P}$ is positive-definite and
$\Tilde{\mathcal{M}}:= \Tilde{\mathcal{P}}^\dagger\Tilde{\mathcal{P}}$
is skew-symmetric\cite{Korn}. For a given orientation of the primal,
$e$, and dual edge, $\Hat{e}$, respectively the Perot operators
$\mathcal{P}^\top$ and $\Tilde{\mathcal{P}}^\dagger$ applied to a
vector field yield the fluxes of the vector field and its
$90^\circ$-rotation through the primal and dual edge respectively.
\begin{equation*}
\Big( \mathrm{P} \boldsymbol{u} \Big)_c
= \frac{1}{A_c} \sum_{e\in E(c)} \frac{\ell_e \ell_{\Hat{e}}}{2} \Vec{n}_{c,e}\cdot\Vec{n}_e \Vec{n}_{c,e} u_e
\end{equation*}
\begin{equation*}
...
= \frac{1}{A_c} \sum_{e\in E(v)} \frac{\ell_e \ell_{\Hat{e}}}{2} \Vec{n}_{e,v}^\bot\cdot \Vec{n}_e \Vec{n}_{e,v}^\bot u_e
\end{equation*}
\begin{equation*}
...
\end{equation*}
**** horizontal momentum advection
The discretization scheme of the horizontal momentum transport is
based on the vector-invariant formulation. The kinetic energy is
computed on cells after an application of the Perot
reconstruction. The flux of the the velocity field rotated by its curl
is approximated by multiplying the curl on dual cells with the Perot
reconstructed velocity field and then apply the Perot operator
$\Tilde{\mathcal{P}}^\dagger$.
\begin{subequations}
\begin{align*}
...
\end{equation*}
**** TODO Explain handling of mass matrix
** hexagonal C grid
*** operators
**** divergence
\begin{equation*}
...
**** curl
** Fourier symbols
*** TODO Real part of the Fourier symbols seems to be incorrect
There is 
** Implementation
The influence of the horizontal discretization of the equations of
motion on the most unstable modes in a baroclinically unstable flow is
analyzed following the framework of \cite{Barham2018} and summarized
here.  For each of the introduced discretizations the system is
linearized at the baroclinically unstable flow first analyzed by
\cite{Eady} and here projected onto the respective discrete spaces. In analogy
to the continuous system, the linearized discrete systems has constant
coefficients on horizontal planes. The application of a discrete
Fourier transform on each horizontal plane yields an equivalent system
formulated in wavenumber space. Since the system had constant
coefficients on horizontal planes the equations for different
wavevectors are decoupled. Eliminating the vertical velocity and
pressure fields by the use of the continuity equation and hydrostatic
balance equation, for each wavenumber the system is reduced to a
linear ordinary differential equation. The reduced system allows for
an instability analysis based on the corresponding eigenvalue problem.

In this study the implementation of the analysis has been automized by
the use of symbolic computations in the \textit{Julia} programming
language. First the discrete system is assembled for the
baroclinically unstable flow projected onto the respective discrete
spaces and perturbed by symbolic flow fields of order $\epsilon$. The
linearized system is the first order system obtained by (Taylor)
expanding the equations in $\epsilon$. For each wavevector the fields
in the pertubation flow are substituted by plane wave fields of the
given wavevector. The amplitudes form the state variables of the
resulting Fourier transformed system at the given wavevector. The
system of differential equations for the amplitudes at given
wavevector is obtained by factoring out the phase of the respective
prognostic field variable (i.e. equivalently, evaluate the equations at
the origin of the respective prognostic field variable).

Note that the dimension of the state on triangular grids is higher than the
respective state on a quadrilateral grid except for A-grid type of
staggering. On a triangular grid with B-type staggering the horizontal
velocity needs four amplitudes instead of two on a quadrilateral
grid. For B-type staggering the horizontal velocity is determined by
three amplitudes instead of two and the scalar quantities each require
two amplitudes instead of one. On a hexagonal grid with B-type
staggering the horizontal velocity is determined by three amplitudes
instead of two on a quadrilateral grid but the scalar quantities also
need only one amplitude each.

#+CAPTION: Number of amplitudes to determine the horizontal velocity field in wavenumber space (in a horizontal plane)
| Staggering         | A | B | C |
|--------------------+---+---+---|
| quadrilateral mesh | 2 | 2 | 2 |
| triangular mesh    | 2 | 4 | 3 |

#+CAPTION: Number of amplitudes to determine a scalar quantity field in wavenumber space (in a horizontal plane)
| Staggering         | A | B | C |
|--------------------+---+---+---|
| quadrilateral mesh | 1 | 1 | 1 |
| triangular mesh    | 1 | 1 | 2 |
...
@@comment: direction of averaging differs though@@
However, staggering introduces horizontal averaging of
vertical velocity in the vertical advection of momentum. There is less
averaging on C grids than on B grids, but it is fully absent on
collocated (A) grids. All grids need horizontal averaging of the
horizontal advecting velocity for momentum, but A and B grids need it
also for scalars.
@@comment: this is only a statement about the flux form, or?@@
In the vertical direction, ocean circulation models
traditionally use the so called Lorenz staggering (see discussion in
\cite{BellWhite2017}) independent of the horizontal
...
#+BEGIN_COMMENT
The background flow is aligned with the zonal coordinate of the
coordinate system. The alginment between the background flow and the
lattices of the mesh is, therefore, the alignment between the
coordinate system and the lattices of the mesh. However, due to the
invariance of the discretization (convolution operators on the
lattice), it is equivalent to analyze the alignment of the background
flow with the coordinate system instead and fix the alignment between
the coordinate sytem and the lattices of the mesh.
#+END_COMMENT
...
**** TODO Ask Sergey about the suitability of the comment
...
mesh.
#+CAPTION: The figures illustrates that the highest growth rate of an instability on the symmetric axis depends on the alignment of the flow with the mesh. In the upper axis the flow is aligned with ...
#+NAME: fig:ali-sym
[[../plots/ali-sym.png]]
...
[[../plots/dis-bar.png]]
**** DONE Create a plot for illustration
**** DONE Implement biharmonic viscosity
...
*** DONE Create plot with different average flow speeds
...
\begin{equation}
\frac{\mathrm{d} \mathtt{\hat{\eta}}\_{2}\left( t \right)}{\mathrm{d}t} =  - 1000 \left( \frac{4 \mathtt{u̲\hat{⃗}}\_{2,1}\left( t \right) \left( 1 + \_expm1im\left(  - \frac{1}{3} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{2,2}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{2,3}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} + \frac{1}{3} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right)}{ - 1.7321 \mathtt{le}} + \frac{4 \mathtt{u̲\hat{⃗}}\_{1,1}\left( t \right) \left( 1 + \_expm1im\left(  - \frac{1}{3} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{1,2}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{1,3}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} + \frac{1}{3} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right)}{ - 1.7321 \mathtt{le}} + \frac{4 \mathtt{u̲\hat{⃗}}\_{4,1}\left( t \right) \left( 1 + \_expm1im\left(  - \frac{1}{3} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{4,2}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{4,3}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} + \frac{1}{3} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right)}{ - 1.7321 \mathtt{le}} + \frac{4 \mathtt{u̲\hat{⃗}}\_{3,1}\left( t \right) \left( 1 + \_expm1im\left(  - \frac{1}{3} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{3,2}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} - \frac{1}{6} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right) + 4 \mathtt{u̲\hat{⃗}}\_{3,3}\left( t \right) \left( 1 + \_expm1im\left( \frac{1}{6} k \mathtt{le} + \frac{1}{3} \left(  - \frac{1}{2} k \mathtt{le} + \frac{1}{2} l \mathtt{le} \mathtt{sqrt3} \right) \right) \right)}{ - 1.7321 \mathtt{le}} \right)
\end{equation}
...
(transport) is obviously $$ \mathrm{T}=g_xU+g_yV.  $$ The same operator acts
...
$((\mathrm{A}_{xx}B_x+\mathrm{A}_{xy}B_y)\hat{u},
(\mathrm{A}_{xy}B_x+\mathrm{A}_{yy}B_y)\hat{v})^T$. Here, $(B_x, B_y)=\nabla
B=M^2(-\sin{\theta},\cos{\theta})$, and the averaging operators are

\begin{align}
\mathrm{A}_{xx}&=(1/3)((1+\cos(ka))+(1/4)(1+\cos(ka/2+lh))+(1/4)(1+\cos(-ka/2+lh)))\\
\mathrm{A}_{xy}&=(\sqrt{3}/12)(\cos(ka/2+lh)-\cos(-ka/2+lh))\\
\mathrm{A}_{yy}&=(1/4)(2+\cos(ka/2+lh)+\cos(-ka/2+lh)).
\end{align}
Because of
...
although the related cross-contributions ($\mathrm{A}_{xy}$) disappear in
...
$\mathrm{A}={\rm diag(\mathrm{T})}$, the column of ones
$\mathbf{1}=(1,1,\ldots,1)^T$ of dimension $N_z$, and the diagonal
matrix $\mathrm{D}_1=\mathrm{diag}(\mathbf{1})$. Using these matrices, the
system matrix of the linearized Eady problem is written as
\begin{equation}     
\mathrm{S}=-\begin{pmatrix} \mathrm{A} +(M^2/f_0)g_x\cos{\theta}\mathrm{W} & -f_0\mathrm{D}_1+(M^2/f_0)g_y\cos{\theta}\mathrm{W} & g_x\mathrm{P} & gg_x\mathbf{1}\\
    f_0\mathrm{D}_1 +(M^2/f_0)g_x\sin{\theta}\mathrm{W}& \mathrm{A} +(M^2/f_0)g_y\sin{\theta}\mathrm{W} & g_y\mathrm{P} & gg_y\mathbf{1}\\  
    -N^2\mathrm{W} g_x +(\mathrm{A}_{xx}B_x+\mathrm{A}_{xy}B_y)\mathrm{D}_1 & -N^2\mathrm{W} g_y  +(\mathrm{A}_{xy}B_x+\mathrm{A}_{yy}B_y)\mathrm{D}_1  & \mathrm{A} & 0\mathbf{1}\\  
    g_x\Delta z\mathbf{1}^T & g_y\Delta z\mathbf{1}^T &0\mathbf{1}^T & \mathrm{A}(1,1) \end{pmatrix}.
    \label{eq:aS}
\end{equation}
The matrices $\mathrm{W}$ and $\mathrm{P}$ are introduced in
...
Laplacian is $$
\Lop=\mathrm{D}\Gop=\frac{4}{3a^2}(\cos(ka)+cos(ka/2+lh)+cos(-ka/2+lh)-3),
$$ and the vector Laplacian has the same Fourier symbol for the
components of velocity. Biharmonic viscosity and diffusion is
introduced by adding $\kappa \Lop^2$ to $\mathrm{T}$ before the diagonal
matrix $\mathrm{T}$ is computed. Here, $\kappa$ is the coefficient of
...
Laplacian correction $$ \mathrm{T} \to \Top(1-\Lop a^2/6).  $$ This can be
