...
operators. Using the notation of \cite{Danilov2017} (see table ...)
discrete analogues of the gradient of a scalar quantity, the
divergence of a flux, and the curl of the horizontal velocity are
...
freedom allows for spurious modes that need to be dissipated by an
appropriate viscosity operator \cite{???}.

The discrete operators may be interpreted as (the sum of pointwise products of)
...
Fourier symbol of a linear discrete operator is then given by
