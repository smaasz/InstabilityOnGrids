...
collocated at the circumcenters of the triangles. In a finite volume
formulation their values are interpreted as the mean on the
resprective triangles which are \textit{called} cells in this
...
lattices. The mismatch in degrees of freedom allows spurious modes
...
***** flux form
Note that in the flux form the vertical transport of momentum that
needs to be discretized is the derivative of the vertical momentum
flux in the vertical direction.
\begin{align*}
\frac{\partial}{\partial z} (w u) &\approx \frac{ w_{k+1/2} \big( u_{k+1} + u_k \big) - w_{k-1/2} \big( u_{k} + u_{k-1} \big)}{2 \Delta_z}\\
&\approx u_k \frac{ w_{k+1/2} - w_{k-1/2} }{\Delta_z} + \frac{1}{2}\Big(w_{k+1/2}\frac{ u_{k+1} - u_k}{\Delta_z} + w_{k-1/2}\frac{u_k - u_{k-1}}{\Delta_z}\Big)
\end{align*}
In order to eliminate the vertical velocity using the volume
conservation we need to subtract the following term from the momementum transport
\begin{equation*}
\mathrm{Av} \boldsymbol{w} \odot 
\end{equation*}
...
At each edge of the cell the mass flux through that edge is
...
the mass flux through the edge by extrapolating from the upwind cell.
...
The transport of scalar quantities (e.g. bouyancy) is discretized in
the flux form. 
