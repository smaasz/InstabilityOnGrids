...
md"""
The data needed to run the notebook is contained in the git repository `InstabilityOnGrids` hosted on GitHub at: [smaasz/InstabilityOnGrids/data](https://github.com/smaasz/InstabilityOnGrids/tree/main/data).
"""
...
The fully assembled system matrices for the considered discretization on a triangular grid with B-type staggering and C-type staggering are summarized in the following equation.
...
function smallkapprox(fsymbol)
	simplify.(fsymbol; rewriter=nantrigexpand(4)) .|> expand .|> simplify
end
...
resp_instab = !isdownload ? Missing : Downloads.download("https://raw.githubusercontent.com/smaasz/InstabilityOnGrids/refs/heads/main/data/simspub.csv");
...
function vertical_ops()
    ...
    (;
     [Symbol("$(name)f") => Symbolics.build_function(substitute(vop, Dict(M² => √(f₀^2 * N²/ Ri))), z, f₀, N², Ri, θU, β, H; expression=Val{false}) for (name, vop) in pairs(vops)]...
         )
end
...
Grid parameter a (in m): $(@bind sa PlutoUI.Slider([6.25e3, 12.5e3, 25e3]; default=6.25e3, show_value=true))
...
Friction velocity V (in m/s): $(@bind sV PlutoUI.Slider([0, 1e-3, 1e-2]; show_value=true))
""", top=340)
end
...
fsyms = !isdownload ? Missing : let
	...
	path   = joinpath("https://github.com/smaasz/InstabilityOnGrids/raw/refs/heads/main/data/symbols/", savename(config, "jld2"))
...
fsymbols = !isdownload ? Missing : let
    fsymbols = Dict{Symbol, Array{Complex{Num}}}([
        name => fsym[1](z, f₀, N², Ri, θU, β, k, l, a, h) for (name, fsym) in pairs(fsyms) 
	])
...
function plotinstabilities(df, Ri; Nz, θU, β, a, Vᵘ, Vᵇ, hst_scheme)
	...
	default(legendfontsize=16, guidefont=(20, "times", :black), tickfont=(16, "times",:black), framestyle = :box, legendfont=(16,"times"), titlefont=(24,"times"))
	...
	M²= √(N² * f₀^2 / Ri)
	plot!(Ks./fₛ, real.(iωs) .* (sqrt(N²) / abs(M²)), label="ideal", linewidth=4, linestyle=:dot, linecolor=:black)
	
	plot!(legend_column=4, legend=:outerbottom, size=size, leftmargin=Plots.AbsoluteLength(10.0))
	#plot!(legend_wfactor=0.5, extra_kwargs = :subplot)
	p
end
...
# ╔═╡ ca0fa66d-6c70-4353-be25-7131d07a7991
# ╠═╡ disabled = true
#=╠═╡
let
	a = sa
	...
	for (iK, K) in enumerate(Ks)
        k = K * cos(θ)
		l = K * sin(θ)
		S̲ = systemmat(Val(grid_t), fsyms, b, k, l; g, f₀, N², H, Nz, Ri, θU, β, a, Vᵘ, Vᵇ, dissip_scheme=:biharmonic)
		iωs[iK] = eigvals(S̲)[end]
	end
	plot(Ks, real.(iωs))
end
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "a292b31d0c9255f103a417a6138c46cef1c4d7af"
